...
function aceesories({ allproducts }) {
    ...
    <section className="text-gray-600 body-font">
      ...
      <div className="container px-5 py-12 mx-auto">
        <div className="flex flex-wrap m-5">
          {Object.keys(allproducts).length == 0 && <h1>all aceesories are out of stock! stay tuned for new stock...</h1>}
...
export async function getServerSideProps(context) {
  ...
  let allproducts = await Products.find({ category: 'aceesory' });

  let allaceesory = {}
  for (let item of allproducts) {
    if (item.title in allaceesory) {
      console.log('pushing into array array');
      console.log("type is:" + typeof (allaceesory[item.title].color))
      if (!allaceesory[item.title].color.includes(item.color) && item.availableQty > 0) {
        allaceesory[item.title].color.push(item.color)
      }
     
    }
    else {
      allaceesory[item.title] = JSON.parse(JSON.stringify(item))

      if (item.availableQty > 0) {
        console.log('array is created');
        allaceesory[item.title].color = [item.color]
       
      }
    }
  }

console.log(allaceesory);
  return {
    props: { allproducts: JSON.parse(JSON.stringify(allaceesory)) }, // will be passed to the page component as props
  };
}


export default aceesories
