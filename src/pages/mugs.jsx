    ...
    <section className="text-gray-600 body-font">
      ...
      <div className="container px-5 py-12 mx-auto">
        <div className="flex flex-wrap m-5">
          {Object.keys(allproducts).length == 0 && <h1>all mugs are out of stock! stay tuned for new stock...</h1>}
...
export async function getServerSideProps(context) {
  ...
  let allproducts = await Products.find({ category: 'mug' });

  let allmug = {}
  for (let item of allproducts) {
    if (item.title in allmug) {
      console.log('pushing into array array');
      console.log("type is:" + typeof (allmug[item.title].color))
      if (!allmug[item.title].color.includes(item.color) && item.availableQty > 0) {
        allmug[item.title].color.push(item.color)
      }
     
    }
    else {
      allmug[item.title] = JSON.parse(JSON.stringify(item))

      if (item.availableQty > 0) {
        console.log('array is created');
        allmug[item.title].color = [item.color]
...
console.log(allmug);
  return {
    props: { allproducts: JSON.parse(JSON.stringify(allmug)) }, // will be passed to the page component as props
  };
}
